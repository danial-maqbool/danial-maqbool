...
The thing I care most about, and what the public repos are largely about, is
**whether a result is real** — calibration, leakage, evaluation design, and the
distance between a metric that looks good and a decision that holds up.

---

## Public repositories

Each runs on CPU with no API keys and no downloads, and regenerates its own
results with `python run.py`. Every number in every README came from running the
code. **103 tests, CI on Python 3.10–3.12.**
...
worse**, estimated from 87 of 569 samples. That is why the textbook cost-optimal
threshold costs more here than the empirical one. Separately: fitting feature
selection before the split manufactured **0.22 AUC out of pure noise**, and a
planted leak whose *parity* encoded the label defeated single-feature AUC,
correlation and mutual information alike.

### [vecsearch](https://github.com/danial-maqbool/vecsearch)
IVF and product quantization in NumPy, validated against faiss.

Product quantization scored recall@10 of 0.23, which looks broken. Its **score
ratio was 0.91** — the neighbours it returned were within 9% of optimal, for a
32× smaller index. Judging it on recall alone leads to the wrong decision.
Profiling then showed the reference index losing to a flat scan; restructuring
the loop from per-query to per-partition gave **4× with bit-identical output**,
pinned by a test.

### [rag-eval](https://github.com/danial-maqbool/rag-eval)
A retrieval evaluation harness.

Fixed-size chunking without overlap made **3–5 of 27 questions unanswerable** —
not badly ranked, impossible, because the answer span was cut across a boundary.
Everyone says "use overlap"; this counts the cost of not doing it. Ranking
metrics and BM25 are written out rather than imported, so the definitions are
inspectable.
...
Structure gets repaired, meaning never does: `"5.1"` becomes `5.1`, `"five point
one"` is rejected, and a missing required field is never filled with a default.
...
The repos are deliberately small and deliberately honest. Where a result did not
hold up, the README says so:
